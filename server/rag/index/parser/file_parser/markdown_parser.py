...
class AsyncTextParser:
    ...
    async def update_doc_status(self, doc_id: int, doc_status: int) -> None:
        logger.info(f"[FILE_CONTENT] update_doc_status, doc_id: {doc_id}, doc_status: {doc_status}")
    ...
    async def add_local_file_chunk(self, file_id: int, chunk_text_vec: List[str], start_index: int) -> None:
        ...
        async with aiosqlite.connect(self.sqlite_db_path) as db:
            ...
            for chunk in chunk_text_vec:
                chunks_to_add.append((file_id, chunk_index, chunk, len(chunk), timestamp, timestamp))
                chunk_index += 1
    ...
    async def add_content(self, doc_id: int, content: str, url: str) -> None:
        ...
        logger.info(f"[FILE_CONTENT] add_content begin, doc_id: {doc_id}, begin_time: {begin_time}")
        ...
        for start in range(0, len(chunk_text_vec), self.ADD_BATCH_SIZE):
            ...
            try:
                with self.distributed_lock.lock():
                    ret = await document_embedder.aadd_local_file_embedding(doc_id, url, batch, self.doc_source)
                    if ret:
                ...
                with self.distributed_lock.lock():
                    ...
                    async with aiosqlite.connect(self.sqlite_db_path) as db:
                        ...
                        await db.execute(
                            ...
                            (doc_id, self.doc_source, json.dumps(embedding_id_vec), timestamp, timestamp)
                        )
                        await db.commit()
            except Exception as e:
                logger.error(f"Process distributed_lock exception: {e}")
        ...
        logger.warning(f"[FILE_CONTENT] add_content end, end_time: {end_time}, timecost: {timecost}")

    async def delete_content(self, doc_id: int) -> None:
        begin_time = int(time.time())
        logger.info(f"[FILE_CONTENT] delete_content begin, doc_id: {doc_id}, begin_time: {begin_time}")
        ...
        end_time = int(time.time())
        timecost = end_time - begin_time
        logger.warning(f"[FILE_CONTENT] delete_content end, end_time: {end_time}, timecost: {timecost}")
