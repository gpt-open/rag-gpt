...
class AsyncCrawlerSiteContent:
    def __init__(self, domain_list: List[str], doc_source: int) -> None:
        logger.info(f"[CRAWL_CONTENT] init, domain_list: {domain_list}, doc_source: {doc_source}")
    ...
    async def fetch_page(self, session: aiohttp.ClientSession, doc_id: int, url: str) -> Optional[str]:
    ...
    async def parse_content(self, html_text: str, url: str) -> List[str]:
        ...
        try:
            ...
            logger.error(f"[CRAWL_CONTENG] parse_content, url:'{url}', Error processing content exception: {e}")
            return []
    ...
    async def update_doc_status(self, doc_id_list: List[int], doc_status: int) -> None:
            ...
            try:
                with self.distributed_lock.lock():
                    if self.doc_source == FROM_SITEMAP_URL:
                        await db.execute(
                            "UPDATE t_sitemap_url_tab SET doc_status = ?, mtime = ? WHERE id IN ({placeholders})".format(
                                placeholders=','.join(['?' for _ in doc_id_list])
                            ), [doc_status, timestamp] + doc_id_list
                        )
                    else:
                        await db.execute(
                            "UPDATE t_isolated_url_tab SET doc_status = ?, mtime = ? WHERE id IN ({placeholders})".format(
                                placeholders=','.join(['?' for _ in doc_id_list])
                            ), [doc_status, timestamp] + doc_id_list
                        )
                    await db.commit()
    ...
    async def get_existing_content_md5(self, doc_id_list: List[int]) -> Dict[int, str]:
        """
        Fetch existing content_md5 from the database for the provided doc_id_list.
        """
        logger.info(f"[CRAWL_CONTENT] get_existing_content_md5, doc_id_list: {doc_id_list}")
        sql = ''
        if self.doc_source == FROM_SITEMAP_URL:
            sql = "SELECT id, content_md5 FROM t_sitemap_url_tab WHERE id IN ({})".format(', '.join('?' for _ in doc_id_list))
        else:
            sql = "SELECT id, content_md5 FROM t_isolated_url_tab WHERE id IN ({})".format(', '.join('?' for _ in doc_id_list))
    ...
    def compare_contents(self,
        existing_contents_md5: Dict[int, str],
        fetched_contents: Dict[int, List[str]]
    ...
    async def compare_and_update_contents(self,
        ...
        existing_contents_md5 = await self.get_existing_content_md5(list(fetched_contents.keys()))
        updated_contents, unchanged_doc_ids = self.compare_contents(existing_contents_md5, fetched_contents)
    ...
    async def process_updated_contents(self,
        updated_contents: Dict[int, List[str]],
        url_dict: Dict[int, str]
    ) -> None:
        """
        ...
        for doc_id, chunk_text_vec in updated_contents.items():
            ...
            content_update_queries.append((content, content_length, content_md5, 3, timestamp, doc_id))
        ...
        try:
            with self.distributed_lock.lock():
                records_to_add, records_to_update = await document_embedder.aadd_document_embedding(data_for_embedding, self.doc_source)
                async with aiosqlite.connect(self.sqlite_db_path) as db:
                    await db.execute("PRAGMA journal_mode=WAL;")
                    ...
                    if records_to_update:
                        if self.doc_source == FROM_SITEMAP_URL:
                            await db.executemany("UPDATE t_sitemap_url_tab SET doc_status = 4, mtime = ? WHERE id = ?", records_to_update)
                        else:
                            await db.executemany("UPDATE t_isolated_url_tab SET doc_status = 4, mtime = ? WHERE id = ?", records_to_update)
                    await db.commit()
        except Exception as e:
            logger.error(f"process distributed_lock exception: {e}")

    async def update_unchanged_contents_status(self, unchanged_doc_ids: List[int]) -> None:
        """
        ...
        async with aiosqlite.connect(self.sqlite_db_path) as db:
            ...
            try:
                with self.distributed_lock.lock():
                    async with aiosqlite.connect(self.sqlite_db_path) as db:
                        if self.doc_source == FROM_SITEMAP_URL:
                            await db.execute("UPDATE t_sitemap_url_tab SET doc_status = 4 WHERE id IN ({})".format(', '.join('?' for _ in unchanged_doc_ids)), unchanged_doc_ids)
                        else:
                            await db.execute("UPDATE t_isolated_url_tab SET doc_status = 4 WHERE id IN ({})".format(', '.join('?' for _ in unchanged_doc_ids)), unchanged_doc_ids)
                        await db.commit()
            except Exception as e:
                logger.error(f"process distributed_lock exception: {e}") 
    ...
    async def add_content(self, url_dict: Dict[int, str]) -> None:
        ...
        logger.warning(f"[CRAWL_CONTENT] add_content end!, end_time: {end_time}, timecost: {timecost}")
    ...
    async def _delete_embedding_doc(self, doc_id_vec: List[int]) -> None:
        logger.info(f"[CRAWL_CONTENT] _delete_embedding_doc, doc_id_vec: {doc_id_vec}")
        placeholder = ','.join('?' * len(doc_id_vec))
        async with aiosqlite.connect(self.sqlite_db_path) as db:
            await db.execute("PRAGMA journal_mode=WAL;")

            cursor = await db.execute(f"SELECT embedding_id_list FROM t_doc_embedding_map_tab WHERE doc_source = ? and  doc_id IN ({placeholder})", [self.doc_source] + doc_id_vec)
    ...
    async def delete_content(self, url_dict: Dict[int, str], delete_raw_table: bool = True) -> None:
        ...
        end_time = int(time.time())
        timecost = end_time - begin_time
        logger.warning(f"[CRAWL_CONTENT] delete_content end, delete_raw_table: {delete_raw_table}, end_time: {end_time}, timecost: {timecost}")

    async def process_delete_batch(self, batch: Dict[int, str], delete_raw_table: bool) -> None:
        ...
        if delete_raw_table:
            async with aiosqlite.connect(self.sqlite_db_path) as db:
                ...
                try:
                    with self.distributed_lock.lock():
                        if self.doc_source == FROM_SITEMAP_URL:
                            await db.execute(f"DELETE FROM t_sitemap_url_tab WHERE id IN ({placeholder})", doc_id_vec)
                        else:
                            await db.execute(f"DELETE FROM t_isolated_url_tab WHERE id IN ({placeholder})", doc_id_vec)
                        await db.commit()
                except Exception as e:
                    logger.error(f"process distributed_lock exception: {e}")
    ...
    async def check_and_update_domain_status(self) -> None:
        ...
        async with aiosqlite.connect(self.sqlite_db_path) as db:
            ...
            for domain in self.domain_list:
                ...
                cursor = await db.execute("SELECT domain_status FROM t_sitemap_domain_tab WHERE domain = ?", (domain,))
                row = await cursor.fetchone()
                if row and row[0] != 4:
                    # Step 2: Check if all URLs for the domain have doc_status >= 4
                    cursor = await db.execute(
                        "SELECT COUNT(*) FROM t_sitemap_url_tab WHERE domain = ? AND doc_status < 4", (domain,))
                    ...
                    if count_row[0] == 0:  # If no records have doc_status < 4
                        try:
                            with self.distributed_lock.lock():
                                ...
                                await db.execute(
                                    "UPDATE t_sitemap_domain_tab SET domain_status = ?, mtime = ? WHERE domain = ?", (4, timestamp, domain))
                                await db.commit()
                        except Exception as e:
                            logger.error(f"process distributed_lock exception: {e}")
