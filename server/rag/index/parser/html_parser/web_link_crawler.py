...
class AsyncCrawlerSiteLink:
    ...
    async def check_urls_existence(self, batch_urls: List[str]) -> Tuple[Dict[str, int], List[str]]:
        """
    ...
    async def update_and_insert_urls(self, existing_urls: Dict[str, int], new_urls: List[str]) -> None:
        """
        ...
        updates: List[Tuple[int, int, int, int]] = [(SITEMAP_URL_RECORDED, self.version, timestamp, url_id) for url_id in existing_urls.values()]
        # Prepare data for inserting new URLs
        inserts: List[Tuple[str, str, str, int, str, int, int, int, int]] = [(self.domain, url, '[]', 0, '', SITEMAP_URL_RECORDED, self.version, timestamp, timestamp) for url in new_urls]
    ...
    async def parse_link(self, session: aiohttp.ClientSession, html_text: str, url: str) -> None:
        ...
        for full_link in link_set:
            ...
            if normalized_link not in self.visited_urls and is_same_domain(self.base_url, normalized_link):
    ...
    async def extract_link(self, html_text: str, url: str) -> Set[str]:
        ...
        try:
            ...
            link_set = {urljoin(url, a['href']) for a in soup.find_all('a', href=True)} if soup.body else set()
            return link_set
        except Exception as e:
            logger.error(f"Error processing content from {url}: {str(e)}")
            return set()

    async def crawl_link(self, session: aiohttp.ClientSession, url: str) -> None:
        self.count += 1
            ...
            try:
                with self.distributed_lock.lock():
                    ...
                    await db.execute(
                        ...
                        (SITEMAP_URL_EXPIRED, timestamp, self.domain, SITEMAP_URL_RECORDED)
                    )
                    await db.commit()
            except Exception as e:
                logger.error(f"process distributed_lock exception:{e}")
    ...
    async def run(self) -> None:
        ...
        await self.update_site_domain_status(DOMAIN_STATISTICS_GATHERING_COLLECTED)
        end_time = int(time.time())
        timecost = end_time - begin_time
        logger.warning(f"[CRAWL_LINK] run end! base_url: '{self.base_url}', end_time: {end_time}, timecost: {timecost}")
