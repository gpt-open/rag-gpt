...
async def parse_file_content_async(file_path: str, file_extension: str, file_md5: str, id_url_info: Dict[str, Any]) -> None:
    ...
    if file_loader_obj:
        ...
        if content:
            await text_parser_obj.add_content(doc_id=doc_id, content=content, url=url)
        else:
            ...
            try:
                ...
                try:
                    with diskcache_lock.lock():
                        cur.execute('UPDATE t_local_file_tab SET doc_status = ?, content_md5 = '' WHERE id = ?',
...
async def add_files_limited_by_semaphore(file_data: List[Dict[str, str]], id_url_info: Dict[str, Any], max_concurrent_writes: int = 5) -> None:
    ...
    tasks = [loop.create_task(semaphore_write(data, id_url_info)) for data in file_data]
    await asyncio.gather(*tasks)

    for data in file_data:
        await parse_file_content_async(data['file_path'], data['file_extension'], data['file_md5'], id_url_info)


def add_local_file_info(file_data: List[Dict[str, str]], id_url_info: Dict[str, Any], max_concurrent_writes: int = 5):
    """Synchronously calls the asynchronous save function to write files with limited concurrency."""
    logger.info(f"[DOWNLOAD FILE] add_local_file_info begin, id_url_info: {id_url_info}")
    beg_time = int(time.time())
    loop = asyncio.new_event_loop()
    asyncio.set_event_loop(loop)
    loop.run_until_complete(add_files_limited_by_semaphore(file_data, id_url_info, max_concurrent_writes))
    loop.close()
    timecost = int(time.time()) - beg_time
    logger.warning(f"[DOWNLOAD FILE] add_local_file_info end, timecost is {timecost}")
...
def submit_local_file_list():
    ...
    try:
        for file_ in file_list:
            ...
            save_directory = os.path.join(STATIC_DIR, LOCAL_FILE_DOWNLOAD_DIR, day_folder, unique_folder)
        ...
        cur.execute(f"SELECT content_md5 FROM t_local_file_tab WHERE doc_status = 4 and content_md5 IN ({placeholders})", tuple(md5_set))
        ...
        cur.execute(f"SELECT id, url, content_md5 FROM t_local_file_tab WHERE content_md5 IN ({placeholders})", tuple(md5_set))
        rows = cur.fetchall()
        ...
        Thread(target=add_local_file_info, args=(file_data, id_url_info, MAX_CONCURRENT_WRITES)).start()
...
def get_local_file_list():
    ...
    try:
        ...
        if file_id_list:
            ...
            cur.execute(f"SELECT id, url, origin_file_name, file_type, content_length, doc_status, ctime, mtime FROM t_local_file_tab WHERE id IN ({placeholders})", file_id_list)
        else:
            cur.execute("SELECT id, url, origin_file_name, file_type, content_length, doc_status, ctime, mtime FROM t_local_file_tab")
...
def delete_local_file_info(file_dict: Dict[int, str]) -> None:
    logger.info(f"[DOWNLOAD FILE] delete_local_file_info beg, file_dict: {file_dict}")
    beg_time = int(time.time())
    loop = asyncio.new_event_loop()
    asyncio.set_event_loop(loop)
    ...
    loop.close()
    timecost = int(time.time()) - beg_time
    logger.warning(f"[DOWNLOAD FILE] delete_local_file_info end, timecost is {timecost}")
...
def delete_local_file_list():
    ...
    try:
        ...
        placeholders = ', '.join(['?'] * len(file_id_list))
        cur.execute(f"SELECT id, file_path FROM t_local_file_tab WHERE id IN ({placeholders})", file_id_list)
        ...
        return {'retcode': 0, 'message': 'Started deleting the local file list embeddings.', 'data': {}}
    except Exception as e:
...
def get_local_file_sub_content_list():
    ...
    try:
        ...
        cur.execute('SELECT COUNT(*) FROM t_local_file_chunk_tab WHERE file_id = ?', (file_id,))
