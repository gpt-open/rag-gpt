...
sitemaps_bp = Blueprint('sitemaps', __name__, url_prefix='/open_kf_api/sitemaps')
...
def submit_crawl_site():
    ...
    if not site or not timestamp:
        return {'retcode': -20000, 'message': 'site and timestamp are required', 'data': {}}

    if not is_valid_url(site):
        logger.error(f"site: '{site} is not a valid URL!")
        return {'retcode': -20001, 'message': f"site: '{site}' is not a valid URL", 'data': {}}
    ...
    try:
        ...
        cur.execute("SELECT id, version FROM t_sitemap_domain_tab WHERE domain = ?", (domain,))
        domain_info = cur.fetchone()

        if domain_info and timestamp <= domain_info["version"]:
            return {'retcode': -20001, 'message': f'New timestamp: {timestamp} must be greater than the current version: {domain_info["version"]}.', 'data': {}}
        ...
        try:
            with diskcache_lock.lock():
                if domain_info:
                    ...
                    cur.execute("UPDATE t_sitemap_domain_tab SET version = ?, domain_status = 1, mtime=? WHERE id = ?", (timestamp, int(time.time()), domain_id))
                else:
                    # Insert new domain record
                    cur.execute("INSERT INTO t_sitemap_domain_tab (domain, domain_status, version, ctime, mtime) VALUES (?, 1, ?, ?, ?)", (domain, timestamp, int(time.time()), int(time.time())))
        ...
        return {'retcode': 0, 'message': 'Site submitted successfully for crawling.', 'data': {}}
    except Exception as e:
        return {'retcode': -30000, 'message': f'An error occurred: {e}', 'data': {}}
    finally:
        if conn:
...
def get_crawl_site_info():
    ...
    try:
        ...
        if site:
            ...
            cur.execute("SELECT * FROM t_sitemap_domain_tab WHERE domain = ?", (domain,))
        else:
        ...
        if rows:
            ...
            return {'retcode': 0, 'message': 'Success', 'data': {'sites_info': sites_info}}
        else:
            return {'retcode': -20001, 'message': 'No site information found', 'data': {}}
    except Exception as e:
    ...
    try:
        ...
        if site is not None:
            if not is_valid_url(site):
                ...
                return {'retcode': -20001, 'message': f"Provided site: '{site}' is not a valid URL.", 'data': {}}

            domain = urlparse(site).netloc
            logger.info(f"Fetching URL list for domain: '{domain}'")
            cur.execute("SELECT domain_status FROM t_sitemap_domain_tab WHERE domain = ?", (domain,))
            domain_status_row = cur.fetchone()
            if domain_status_row:
                response_data['domain_status'] = domain_status_row['domain_status']
            cur.execute("SELECT id, url, content_length, doc_status, version, ctime, mtime FROM t_sitemap_url_tab WHERE domain = ?", (domain,))
        else:
            logger.info("Fetching URL list for all domains.")
            cur.execute("SELECT id, url, content_length, doc_status, version, ctime, mtime FROM t_sitemap_url_tab")
...
def async_crawl_content_task(domain_list: List[str], url_dict: Dict[int, str], task_type: int) -> None:
    """
...
def check_crawl_content_task(f: FlaskViewFunction) -> FlaskViewFunction:
    ...
    def decorated_function(*args: Any, **kwargs: Any) -> Dict[str, Any]:
        ...
        if not id_list or not isinstance(id_list, list) or len(id_list) == 0:
            return {'retcode': -20000, 'message': 'Invalid or missing id_list parameter'}
        ...
        try:
            ...
            cur.execute(f"SELECT id, domain, url FROM t_sitemap_url_tab WHERE id IN ({placeholders})", id_list)
            rows = cur.fetchall()

            if len(rows) != len(id_list):
                missing_ids = set(id_list) - set(row[0] for row in rows)
                return {'retcode': -20001, 'message': f'The following ids do not exist: {missing_ids}', 'data': {}}
            ...
            for domain in domain_list:
                ...
                if domain_info and domain_info["domain_status"] < DOMAIN_PROCESSING:
                    try:
                        with diskcache_lock.lock():
                            cur.execute("UPDATE t_sitemap_domain_tab SET domain_status = ?, mtime = ? WHERE domain = ?",
                                (DOMAIN_PROCESSING, timestamp, domain)
                            )
                            conn.commit()
                            logger.info(f"Updated domain_status to {DOMAIN_PROCESSING} for domain: '{domain}'")
                    except Exception as e:
                        logger.error(f"Process discache_lock exception:{e}")
                        return {'retcode': -30000, 'message': f'An error occurred: {e}', 'data': {}}
        except Exception as e:
...
def add_crawl_url_list():
    ...
    Thread(target=async_crawl_content_task, args=(domain_list, url_dict, ADD_SITEMAP_CONTENT)).start()
    return {'retcode': 0, 'message': 'Started processing the URL list.', 'data': {}}
...
def delete_crawl_url_list():
    ...
    url_dict = request.url_dict
    # Use threading to avoid blocking the Flask application
    Thread(target=async_crawl_content_task, args=(domain_list, url_dict, DELETE_SITEMAP_CONTENT)).start()
    return {'retcode': 0, 'message': 'Started deleting the URL list embeddings.', 'data': {}}
...
def update_crawl_url_list():
    ...
    url_dict = request.url_dict
    # Use threading to avoid blocking the Flask application
    Thread(target=async_crawl_content_task, args=(domain_list, url_dict, UPDATE_SITEMAP_CONTENT)).start()
    return {'retcode': 0, 'message': 'Started updating the URL list embeddings.', 'data': {}}
...
def get_crawl_url_sub_content_list():
    ...
    try:
        ...
        cur.execute('SELECT content FROM t_sitemap_url_tab WHERE id = ?', (url_id,))
        row = cur.fetchone()
        if not row:
